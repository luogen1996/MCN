...
Notably, the weights `DarkNet53-yolov3 (finetuned on RefCOCO(train))`  are further finetuned on the *train* set of RefCOCO, which will achieve better results for RefCOCO and RefCOCO+.  (it's not suitable for RefCOCOg, whose images from the *val+test* set  also appears in the *train* set).  Meanwhile, the weights of darknet pretrained on the whole *train* set of MS-COCO are also released as following, which will boost the  performance of REC around 2~3% in practice.   Notbly, we release it for training on other datasets like referit, but do not advise to use it for RefCOCO, RefCOCO+ and RefCOCOg.  **(it is incorrect to use the COCO pre-trained backbone on RefCOCO, RefCOCO+, and RefCOCOg datasets)** .  
